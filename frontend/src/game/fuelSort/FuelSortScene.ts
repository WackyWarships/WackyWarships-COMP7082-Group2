...
let overlayHintShownOnce = false;
...
export class FuelSortScene extends Phaser.Scene {
    ...
    // ================================================
    // === MULTIPLAYER OVERLAY SUPPORT ========
    // ================================================
    private asOverlay: boolean = false;
    private overlayLobbyId?: string;
    private overlayTurnId?: number;
    // ================================================
    ...
    init(data?: {
        ...
        this.asOverlay = !!data?.asOverlay;
        if (this.asOverlay) {
            this.overlayLobbyId = data?.lobbyId;
            this.overlayTurnId = data?.turnId;
        }
    }
    // ================================================
    ...
    create(): void {
        ...
        // Standalone: show hint overlay + countdown.
        // Overlay (battle): start gameplay immediately so ATTACK
        // → minigame appears right away.
    ...
    private createUI(width: number, height: number): void {
        ...
        this.descriptionText = this.add.text(
            width / 2,
            170,
            'Sort the fuel in the given time frame to fire your weapon',
            {
                fontSize: '20px',
                color: '#bbbbbb',
                fontFamily: 'Arial, sans-serif',
    ...
    private createPauseModal(): void {
        ...
        this.pauseModal.add([backdrop, panel, title, content, resumeButton, forfeitButton]);
    }
    ...
    private handleWin(): void {
        ...
        //When Fuel Sort finishes in multiplayer overlay mode, send the result back to Game.ts (and then to the server) instead of returning to MainMenu.
    ...
    private showHintOverlay(): void {
        // In overlay mode we skip the hint screen and just start
        // immediately so ATTACK feels responsive.
        if (this.asOverlay) {
            if (overlayHintShownOnce) {
                // Already showed once for this client
                this.clearHintOverlay();
                this.startGameplay();
                return;
            }
            // Mark that we've now shown it once for overlays
            overlayHintShownOnce = true;
        }
        ...
        const panel = this.add.rectangle(
            width / 2,
            height / 2,
            Math.min(width * 0.75, 680),
            Math.min(height * 0.6, 460),
            0x252543,
            0.95
        );
        ...
        // Small delay so the original ATTACK click that opened
        // the minigame does NOT immediately start the countdown
        ...
        // Auto-start after ~1.6s if the player doesn’t tap
    ...
    private handleTimeUp(): void {
        ...
        // If this Fuel Sort instance is running as a multiplayer overlay, report a failure result to Game.ts instead of returning to MainMenu.
    ...
    // ================================================
    // === FORFEIT BEHAVIOUR ==================
    // - Treat as immediate failure for this turn.
    // - Same as time-up, but triggered by user choice.
    // ================================================
    private handleForfeit(): void {
        ...
        // Close pause modal if it's open
        if (this.pauseModal) {
            this.pauseModal.destroy();
            this.pauseModal = undefined;
            this.isPaused = false;
    ...
    // Draws a different background when the minigame is in multiplayer overlay mode (dark rounded box) instead of the normal gradient background.
    private drawBackground(width: number, height: number): void {
        ...
        if (this.asOverlay) {
            ...
            this.backgroundGraphics.fillRoundedRect(marginX, marginY, w, h, 24);
        } else {
    ...
    private getTubeScale(width: number, height: number, totalTubes: number): number {
        ...
        const baseScale = Phaser.Math.Clamp(
            Math.min(width / 900, height / 1200),
            0.5,
            1.1
        );
        const minScale = 0.45;
    ...
    private calculateTubePositions(
        ...
        rowCounts.forEach((count, rowIndex) => {
            ...
            const maxStart =
                width - horizontalMargin - tubeWidth / 2 - totalRowWidth;
    ...
    private getResponsiveFontSize(base: number, width: number, height: number): number {
        const scaleFactor = Phaser.Math.Clamp(
            Math.min(width / 768, height / 1024),
            0.65,
            1.25
        );
    ...
    private generateLevelFromConfig(
